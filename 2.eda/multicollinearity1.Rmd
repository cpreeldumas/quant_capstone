...
library(stargazer)
library(sjPlot)
library(corrplot) #I added this, and the test will start from line 225.
...
```{r}
#The multicollinearity test start from here!
...
vif(model)
```
# There is no VIF is more than 5, no strong evidence for multicollinearity, but we still want to use correaltion matrix to explore more trends.
...
variables <- bls_data_common_pre[, c("unemployment_rate", "treatment", "renters_occupied_percentage", 
                                "black_or_african_american_percentage","hispanic_or_latino_percentage",
                                     ...
                                     "laborforce_16over","health_spending", 
...
p.cor_matrix <- cor.mtest(variables)
...
```
```{r}
# We can tell that the concerning correlation coefficients are (more than 0.7 is a high value)：
  #Bachelors_percentage & Median_income : 0.71
#It is understandable that these two variables are corelated with each other.
#So we remove Median_income to see the new correlationship.
...
# matrix of the p-value of the correlation
p.cor_matrix_new <- cor.mtest(variables_new)
...
```
# There is no coefficient more than 0.7 this time, so we pass the multicollinearity check.
