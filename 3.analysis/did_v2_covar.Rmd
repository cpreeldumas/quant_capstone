...
bls_data_common_pre <- bls_data_did_nodc %>% 
  ...
  mutate(treat_date_interact = treatment * new_year_mo)

bls_data_common_pre %>% 
  group_by(treatment) %>% tally()

bls_data_common_pre %>% 
  group_by(treatment, state) %>% tally()

pre_testlm <- lm(unemployment_rate ~ treatment + new_year_mo + treat_date_interact + 
                   total_population +
                   renters_occupied_percentage +
                   black_or_african_american_percentage + 
                   hispanic_or_latino_percentage +
...
pre_test <- plm(unemployment_rate ~ treatment,
                data = bls_data_common_pre,
                model = "within",
                index = c("new_year_mo"))
summary(pre_test)
