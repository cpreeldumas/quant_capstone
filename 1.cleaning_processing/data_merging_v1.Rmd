...
```{r}
covariates_census <- read_csv("~/Documents/Coding Projects/quant_capstone/data_raw/covariates_census_v13.csv") %>%
  separate(GEOID, into = c("state_fips", "county_fips"), -3) %>%
...
```{r}
BLS_seas <- read_csv("~/Documents/Coding Projects/quant_capstone/data_raw/BLS_seas_v3.csv") %>%
  mutate(date = format(date, '%Y-%m')) %>%
  filter(date >= "2020-07") 
...
data2 <- left_join(data, covariates_covid_statespend, by = c("date", "county_fips", "state_fips"))
data3 <- left_join(data2, covariates_laborforce,  by = c("state_fips", "county_fips"))
data4 <- left_join(data3, covariates_census,  by = c("county"))
...
write.csv(data5, "~/Documents/Coding Projects/quant_capstone/data_clean/data_v5.csv", row.names=FALSE)
