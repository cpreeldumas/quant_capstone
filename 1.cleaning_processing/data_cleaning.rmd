...
BLS_county_employment_v3 %>% 
  mutate(date = as.Date(paste(year, month, "01", sep = "-"))) %>%
  ggplot(aes(x=date, y=Unemployment_Rate, colour=control, group=control)) +        geom_smooth(aes(group = control), method = "auto", size = 2, se = FALSE) +
  geom_vline(xintercept = as.numeric(as.Date("2021-08-26")), linetype=4)


# Descriptive Statistics

str(BLS_county_employment_v3)


# Export CV

write.csv(BLS_county_employment_v3, "Documents/Coding Projects/quant_capstone/data_raw/BLS_county_employment_v3.csv", row.names=FALSE)
